...
const progress = [
    { label: "Lead created", detail: "Lead profile added to CRM", status: "Done" },
    { label: "Contacted", detail: "Initial outreach ready for agent", status: "Done" },
    { label: "Qualified", detail: "Waiting for admin or agent review", status: "Current" },
    { label: "Proposal", detail: "Next step after qualification", status: "Next" },
];
...
export default function AdminLeads() {
    ...
    const selectedLead = filteredLeads.find((lead) => lead._id === selectedLeadId) || filteredLeads[0] || null;
        ...
        <AdminLayout>
                            ...
                            {filteredLeads.map((lead) => (
                                ...
                                >
                                    ...
                                    <span className="shrink-0 text-right">
                                        <span className="block text-xs font-semibold text-white/45">{lead.source}</span>
            ...
            {isPlacesOpen && (
                <div className="modal-backdrop-enter fixed inset-0 z-50 flex items-center justify-center bg-black/60 px-4 backdrop-blur-sm">
                    <section className="modal-panel-enter flex max-h-[88vh] w-full max-w-[42rem] flex-col rounded-lg border border-white/10 bg-[#0d1018] shadow-2xl shadow-black/40">
                        ...
                        <form className="flex flex-col gap-3 border-b border-white/10 p-4 md:flex-row" onSubmit={(event) => {
                            event.preventDefault();

                            if (placesQuery.trim()) {
                                searchPlacesMutation.mutate(placesQuery);
                            }
                        }}>
                            <label className="flex h-11 min-w-0 flex-1 items-center gap-3 rounded-lg border border-white/10 bg-black/20 px-3 text-white/45 focus-within:border-[#842cff] focus-within:ring-2 focus-within:ring-[#842cff]/20">
                                ...
                                <input
                                    ...
                                    placeholder="Dentists in Austin, SaaS companies in Taipei..."
                                />
                            </label>
                            <button
                                className="flex h-11 items-center justify-center gap-2 rounded-lg bg-[linear-gradient(135deg,#842cff,#4a0ebd)] px-4 text-sm font-semibold text-white transition hover:brightness-110"
                                type="submit"
                            >
                                <FiMapPin className="size-4" aria-hidden="true" />
                                Search
                        ...
                        <div className="content-scroll overflow-y-auto p-4">
                            {searchPlacesMutation.isError && (
                                <p className="text-sm text-red-200">
                                    Google Places search failed. Check `GOOGLE_PLACES_API_KEY` in `backend/.env`.
                                </p>
                            )}
                            {placeResults.length === 0 && !searchPlacesMutation.isError && (
                                <p className="text-sm text-white/45">Search results will appear here.</p>
                            )}
                                ...
                                {placeResults.map((place) => (
                                    ...
                                    >
                                        <div className="flex items-start justify-between gap-3">
                                            ...
                                            <button
                                                className="flex h-9 shrink-0 items-center gap-2 rounded-lg border border-white/10 bg-white/[0.06] px-3 text-xs font-semibold text-white/70 transition hover:bg-white/10 hover:text-white"
                                                type="button"
                                                onClick={() => importPlacesMutation.mutate([place])}
                                            >
